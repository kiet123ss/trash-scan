...
class RecycleDetector:
    ...
    def track_image(self, img_bgr: np.ndarray):
        ...
        # conf=0.5: Balance giữa ít flickering và không bị duplicate box
        # iou=0.4: NMS nghiêm ngặt hơn để loại box trùng
        results = self.model.track(img_bgr, imgsz=640, conf=0.5, iou=0.4, tracker="bytetrack.yaml", persist=True, verbose=False)[0]
